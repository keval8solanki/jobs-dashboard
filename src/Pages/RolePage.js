...
function RolePage({ match }) {
	...
	const deleteHandler = async () => {
		if (role && role.admins.length === 0) {
			console.log(role.admins.length)
			try {
				await axios.delete(`${API_URI}role/${id}`, { withCredentials: true })
				setIsDeleted(true)
			} catch (err) {
				setIsDeleted(false)
			}
			...
			toast.error(
				<p style={{ margin: '0px', padding: '0px' }}>
					Cannot Delete Role
					<br />
					as it is assigned to admin(s)
					<br />
					Please unassign and try again
				</p>
			)
		}
	}
