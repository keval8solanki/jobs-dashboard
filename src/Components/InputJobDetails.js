...
function InputJobDetails({ match }) {
	...
	const sendDataHandler = async () => {
		...
		const request =
			type === 'edit'
				? axios.put(`${API_URI}${id}`, data, { withCredentials: true })
				: axios.post(`${API_URI}create`, data, { withCredentials: true })
		try {
		...
		const list = data.map((item) => {
			return (
				<Element key={item}>
					...
					<Icon
						onClick={() => deleteItemHandler(data, item, setCurrState)}
						src={CloseSVG}
					/>
				</Element>
			)
				...
				<Section>
					...
					<InputContainer>
						...
						<InputWithButton
							...
							<CustomInput
								value={eligibilitiesVal}
								onChange={(e) => setEligibilitiesVal(e.target.value)}
								type='text'
							/>
							<AddButton>Add</AddButton>
						</InputWithButton>
						<Ul>{renderList(eligibilities, setEligibilities)}</Ul>
					</InputContainer>
					<InputContainer>
						...
						<Ul>{renderList(responsibilities, setResponsibilities)}</Ul>
					</InputContainer>
				</Section>
			</Card>
